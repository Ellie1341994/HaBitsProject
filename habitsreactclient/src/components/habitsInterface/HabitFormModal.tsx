...
export function HabitFormModal(_props: any) {
  ...
  async function handleFormSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    let cond1: boolean = form.EndHour.value < form.StartHour.value;
    let cond2: boolean =
    ...
    try {
      ...
      ](URL, data, { headers: headers }).catch();
      console.log(response);
    ...
    >
      ...
      >
        <ModalHeader fontFamily="serif" rounded="md" bgColor={bgColor}>
          {_props.type === "Edit" ? "Edit " + _props.name : "Habit"}
        </ModalHeader>
